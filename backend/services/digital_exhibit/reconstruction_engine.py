...
from common.models import DigitalReconstruction, WaterHeritageSite, FunctionalRestoration
from common.config import channels
from common.redis_client import pubsub
from geoalchemy2.shape import to_shape
from shapely.geometry import mapping

logger = logging.getLogger("digital_exhibit.engine")
...
class DigitalReconstructionPipeline:
    ...
    def run_full_pipeline(self, db: Session, site_id: int,
                          photo_urls: List[str],
                          method: str = '摄影测量',
        ...
        if not site:
            raise ValueError(f"遗址不存在: site_id={site_id}")

        pubsub.publish(channels.DIGITAL_RECONSTRUCTION_REQUESTED, {
            "event_type": "digital_reconstruction_requested",
            "site_id": site_id,
            "method": method,
            "photo_count": len(photo_urls),
            "generate_vr": generate_vr,
            "timestamp": time.time(),
            "source": "digital_exhibit",
        })

        existing = db.query(DigitalReconstruction).filter(
            DigitalReconstruction.site_id == site_id
        ).first()

        if existing:
            reconstruction = existing
            reconstruction.reconstruction_status = "处理中"
            reconstruction.photos_uploaded_count = len(photo_urls)
            reconstruction.reconstruction_method = method
            reconstruction.reconstruction_log = reconstruction_log
            reconstruction.model_metadata = None
            reconstruction.glb_model_url = None
            reconstruction.gltf_model_url = None
            reconstruction.vr_experience_url = None
            reconstruction.point_cloud_count = None
            reconstruction.mesh_face_count = None
            reconstruction.texture_resolution = None
        else:
            reconstruction = DigitalReconstruction(
                site_id=site_id,
                photos_uploaded_count=len(photo_urls),
                reconstruction_method=method,
                reconstruction_status="处理中",
                reconstruction_log=reconstruction_log,
                overlay_with_irrigation=False,
            )
        ...
        try:
            ...
            if not all_valid:
                ...
                reconstruction.reconstruction_status = "失败"
                reconstruction.reconstruction_log = reconstruction_log
            ...
            reconstruction.point_cloud_count = dense_data["dense_points_count"]
            ...
            reconstruction.mesh_face_count = mesh_data["decimated_face_count"]
            ...
            reconstruction.texture_resolution = texture_data["texture_resolution"]
            ...
            reconstruction.gltf_model_url = export_result["gltf_model_url"]
            reconstruction.glb_model_url = export_result["glb_model_url"]
            ...
            if generate_vr:
                ...
                irrigation_geom = None
                try:
                    restoration = db.query(FunctionalRestoration).filter(
                        FunctionalRestoration.site_id == site_id
                    ).first()
                    if restoration and restoration.water_supply_range_geom:
                        shape = to_shape(restoration.water_supply_range_geom)
                        irrigation_geom = mapping(shape)
                except Exception:
                    irrigation_geom = None
                ...
                try:
                    if site.geom:
                        site_shape = to_shape(site.geom)
                        site_geom = mapping(site_shape)
                except Exception:
                    site_geom = None

                if site_geom and irrigation_geom and dense_data.get("point_cloud_stats"):
                    irrigation_overlay = self.exporter.overlay_irrigation_zone(
                        site_geom, irrigation_geom, dense_data["point_cloud_stats"]
                    )
                    reconstruction.overlay_with_irrigation = True
                    model_metadata["stages"]["irrigation_overlay"] = irrigation_overlay

                vr_result = self.exporter.generate_vr_experience(
                    export_result["gltf_model_url"],
                    irrigation_geojson=irrigation_geom,
                    site_metadata={"site_id": site_id, "name": site.name}
                )
                self._record_step(reconstruction_log, 8, "VR体验生成", "success", step_start, {
                    ...
                    "has_irrigation_overlay": reconstruction.overlay_with_irrigation,
                })
                model_metadata["stages"]["vr_experience"] = vr_result
                reconstruction.vr_experience_url = vr_result["vr_experience_url"]

                pubsub.publish(channels.VR_EXPERIENCE_GENERATED, {
                    "event_type": "vr_experience_generated",
                    "site_id": site_id,
                    "reconstruction_id": recon_id,
                    "vr_url": vr_result["vr_experience_url"],
                    "hotspots_count": len(vr_result["hotspots"]),
                    "has_irrigation_overlay": reconstruction.overlay_with_irrigation,
                    "timestamp": time.time(),
                    "source": "digital_exhibit",
            ...
            current_step = 9
            step_start = time.time()
            reconstruction.reconstruction_status = "已完成"
            reconstruction.model_metadata = model_metadata
            ...
            pubsub.publish(channels.DIGITAL_RECONSTRUCTION_COMPLETED, {
                "event_type": "digital_reconstruction_completed",
                "site_id": site_id,
                "reconstruction_id": recon_id,
                "method": method,
                "point_cloud_count": reconstruction.point_cloud_count,
                "mesh_face_count": reconstruction.mesh_face_count,
                "texture_resolution": reconstruction.texture_resolution,
                "glb_model_url": reconstruction.glb_model_url,
                "vr_generated": generate_vr and vr_result is not None,
                "vr_url": reconstruction.vr_experience_url,
                "timestamp": time.time(),
                "source": "digital_exhibit",
            })
            ...
            logger.error(f"重建管线失败 site_id={site_id} step={current_step}: {e}")
