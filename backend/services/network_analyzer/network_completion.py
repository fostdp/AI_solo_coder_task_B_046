...
class HydrologicalNetworkCompletor:
    ...
    def _text_contains_hydrology_keyword(self, text: str) -> bool:
        if not text:
            return False
        for kw in self.HYDROLOGY_KEYWORDS:
            if kw in text:
                return True
        return False
    ...
    def extract_known_hydrology(self, sites: List) -> Dict[str, Any]:
        ...
        for i, site in enumerate(sites):
            name_match = self._text_contains_hydrology_keyword(site.name)
            desc_match = False
            if hasattr(site, 'description') and site.description:
                desc_match = self._text_contains_hydrology_keyword(site.description)
    ...
    def _compute_downstream_score(self, s1, s2) -> float:
        elev1 = self._get_approx_elevation(s1)
        elev2 = self._get_approx_elevation(s2)
        ...
        direction_ok = self._is_downstream_flow(s1, s2) or self._is_downstream_flow(s2, s1)
    ...
    def monte_carlo_network_sampling(self, n_samples: int = 200,
        ...
        def _stats(values: List[float]) -> Dict[str, float]:
            ...
            return {
                'mean': round(mean, 6),
                'std': round(std, 6),
                'p5': round(self._percentile(values, 5), 6),
                'p50': round(self._percentile(values, 50), 6),
                'p95': round(self._percentile(values, 95), 6),
            }

        result = {
            'connectivity': {**_stats(connectivity_samples), 'samples': connectivity_samples},
            'redundancy': {**_stats(redundancy_samples), 'samples': redundancy_samples},
            'cascade_efficiency': {**_stats(cascade_samples), 'samples': cascade_samples},
            'synergy_score': {**_stats(synergy_samples), 'samples': synergy_samples},
