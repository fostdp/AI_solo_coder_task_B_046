...
class DigitalExhibitPanel {
    ...
    show() {
        if (this.sitePanelEl) {
            this.sitePanelEl.style.display = 'block';
        }
    }

    hide() {
        if (this.sitePanelEl) {
            this.sitePanelEl.style.display = 'none';
        }
    }

    updateData(data) {
        if (data && data.site_id) {
            this.loadSite(data.site_id);
        }
    }

    _injectStyles() {
        if (document.getElementById('digital-display-styles')) return;
        const style = document.createElement('style');
        style.id = 'digital-display-styles';
        style.textContent = `
    ...
    _showViewer() {
        ...
        this._emit('reconstructionComplete', { siteId: this.currentSiteId });
    }
